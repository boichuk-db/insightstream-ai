...
Every Phase 1 bullet from the design doc has a task: Eyebrow (Task 1), Button xs (Task 2), StatusSelect + STATUS_CONFIG (Tasks 3–4), Popover + Dropdown/Select/FilterChips refactor (Task 5), Tabs/SegmentedControl/ChoiceCard (Task 6 — StatusTabs deliberately left unrefactored per Task 6 Step 4's YAGNI decision, not delegated to Tabs; corrected here after an earlier draft of this line incorrectly claimed a StatusTabs refactor happened), Drawer/Overlay + Modal refactor (Task 7), ConfirmDialog (Task 8), CommentThread (Task 9), FormField (Task 10), NavItem (Task 11), WidgetConfigForm (Task 12). The file-ownership rule's exception (`lib/colors.ts` keeping its old export) is called out both in the Pre-flight section and inline in Task 3.
