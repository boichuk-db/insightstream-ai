...
export function AnalyticsOverview({ feedbacks }: AnalyticsProps) {
  const chartData = useMemo(() => {
    ...
    const sentimentGroups: Record<string, { totalScore: number; count: number }> = {};
    ...
    sorted.forEach(fb => {
      ...
      if (fb.sentimentScore !== undefined && fb.sentimentScore !== null) {
        ...
        if (!sentimentGroups[dateKey]) sentimentGroups[dateKey] = { totalScore: 0, count: 0 };
    ...
    const sentimentData = Object.entries(sentimentGroups).map(([date, data]) => ({
      date,
        ...
        <div className="h-[250px] w-full">
          {chartData.sentiment.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData.sentiment} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
      ...
      <div className="bg-brand-surface border border-brand-border/50 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
        ...
        <div className="h-[250px] w-full">
          {chartData.categories.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData.categories} layout="vertical" margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
