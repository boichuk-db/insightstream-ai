...
function applyFilters(
  ...
  if (sortBySentiment) {
    result.sort(
      (a, b) => (a.sentimentScore ?? 0.5) - (b.sentimentScore ?? 0.5),
    );
  }
...
export function KanbanBoard({ initialFeedbacks, projectId }: KanbanBoardProps) {
  ...
  useEffect(() => {
    ...
    initialFeedbacks.forEach((fb) => {
      const status =
        fb.status && COLUMNS.some((c) => c.id === fb.status)
          ? fb.status
          : "New";
  ...
  const allTags = useMemo(() => {
    ...
    initialFeedbacks.forEach((fb) =>
      fb.tags?.forEach((t: string) => tagSet.add(t)),
    );
  ...
  const getExportTitle = useCallback(() => {
    const col =
      exportScope === "all" ? "All Feedbacks" : `${exportScope} — Feedbacks`;
  ...
  const updateStatusMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      alert(
        `Failed to update feedback status: ${err?.response?.data?.message || err.message}`,
      );
    },
  });
  ...
  const reanalyzeMutation = useMutation({
    ...
    onError: (err: any) => {
      alert(
        `Re-analysis failed: ${err?.response?.data?.message || err.message}`,
      );
    },
  });
        ...
        <div className="relative z-20 flex items-center gap-3">
          <Button
            ...
            disabled={
              ...
              displayColumns["Rejected"]?.length === 0
            }
