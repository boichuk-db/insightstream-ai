...
async function testConnection() {
    try {
        ...
        console.log('✅ Успіх! База даних підключена, таблиці (User, Feedback, AuditLog) синхронізовані!');
        const userRepository = data_source_1.AppDataSource.getRepository('User');
        const existing = await userRepository.findOne({ where: { email: 'test@example.com' } });
        if (existing) {
        ...
        console.log(`✅ Тестового користувача успішно збережено в БД з ID: ${newUser.id}`);
    }
    catch (error) {
